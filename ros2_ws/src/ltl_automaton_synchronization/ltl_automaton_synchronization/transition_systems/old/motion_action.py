from ros2_ws.src.ltl_automaton_synchronization.ltl_automaton_synchronization.transition_systems.old.motion_ts import MotionFts, MotionFts2
from ros2_ws.src.ltl_automaton_synchronization.ltl_automaton_synchronization.transition_systems.old.action_mod import ActionModel
        ...
        for reg in self.graph['region'].nodes():
            for act in self.graph['action'].action.keys():
                ...
                for reg_to in self.graph['region'].successors(reg):
                    ...
                    self.add_edge((reg, 'None'), prod_node_to, weight=self.graph['region'][reg][reg_to]['weight'], action= 'goto_'+str(reg_to), marker= 'visited')
            # adding self transitions (none action to waste time)
            self.add_edge((reg, 'None'), (reg, 'None'), weight=self.graph['action'].action['None'][0], action= 'none', marker= 'visited')                
            
                
                
class MotActModel3(DiGraph):
    def __init__(self, mot_fts, act_model):
        DiGraph.__init__(self, region=mot_fts, action=act_model, initial=set(), type='MotActModel')
    ...
    def composition(self, reg, act):
        ...
        if not self.has_node(prod_node):
            ...
            new_label = (self.graph['region'].nodes[reg]['label'], self.graph['action'].action[act]['label'])
            print("before")
            print(new_label)
            print(prod_node)
            print("after")
    ...
    def build_edges(self):
        for reg in self.graph['region'].nodes():
            for act in self.graph['action'].action.keys():
                ...
                for reg_to in self.graph['region'].successors(reg):
                    # add edge between regions (actions None for both nodes)
                    self.add_edge((reg, 'None'), (reg_to, 'None'), weight=self.graph['region'][reg][reg_to]['weight'], action= 'goto_'+str(reg_to), marker= 'visited')
            # adding self transitions (none action to waste time)
            self.add_edge((reg, 'None'), (reg, 'None'), weight=self.graph['action'].action['None']['weight'], action= 'none', marker= 'visited')        
