...
class Draw_MPC_Obstacle(object):
    def __init__(self, robot_states: list, init_state: np.array, target_state: np.array, obstacle: np.array,
        ...
        if obstacle is not None:
            self.obstacle = obstacle[0:3]
            self.obstacle2 = obstacle[3:6]
            self.obstacle3 = obstacle[6:9]
        else:
    ...
    def animation_init(self):
        ...
        self.obstacle_circle2 = plt.Circle(self.obstacle2[:2], self.obstacle2[2], color='g', fill=True)
        self.obstacle_circle3 = plt.Circle(self.obstacle3[:2], self.obstacle3[2], color='g', fill=True)
        self.ax.add_artist(self.obstacle_circle)
        self.ax.add_artist(self.obstacle_circle2)
        self.ax.add_artist(self.obstacle_circle3)
        return self.target_circle, self.target_arr, self.robot_body, self.robot_arr, self.obstacle_circle, self.obstacle_circle2
