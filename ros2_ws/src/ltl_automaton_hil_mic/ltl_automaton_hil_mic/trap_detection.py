...
class TrapDetectionPlugin(object):
    ...
    #--------------------------------------------------------------------------
    def __init__(self, ltl_planner_node, args_dict):
        self.ltl_planner_node = ltl_planner_node
    ...
    #---------------------------------------------
    def trap_check_callback(self, trap_check_req, res):
    ...
    def is_trap(self, ts_state):
        ...
        reachable_states = self.ltl_planner_node.ltl_planner.product.get_possible_states(ts_state)
    ...
    def has_path_to_accept(self, f_s):
        for acc in self.ltl_planner_node.ltl_planner.product.graph['accept']:
            if has_path(self.ltl_planner_node.ltl_planner.product, f_s, acc):
                return True
        return False

    def has_path_to_accept_with_cycle(self, f_s):
        for acc in self.ltl_planner_node.ltl_planner.product.graph['accept_with_cycle']:
            if has_path(self.ltl_planner_node.ltl_planner.product, f_s, acc):
                return True
        return False