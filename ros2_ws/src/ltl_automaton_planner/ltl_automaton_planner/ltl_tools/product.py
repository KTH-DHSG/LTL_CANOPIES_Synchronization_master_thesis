...
class ProdAut(DiGraph):
    def __init__(self, ros_node, ts, buchi, beta=1000):
    ...
    def build_full(self):
        ...
        self.ros_node.get_logger().info('LTL Planner: full product constructed with %d states and %s transitions' %(len(self.nodes()), len(self.edges())))
...
class ProdAut_Run(object):
    ...
    # line, loop in ts
    def __init__(self, ros_node, product, prefix, precost, suffix, sufcost, totalcost):
        self.ros_node = ros_node
        self.prefix = prefix
    ...
    def plan_output(self, product):
        ...
        self.ros_node.get_logger().info('LTL Planner: Prefix plan: ' + str(self.pre_plan))
        self.ros_node.get_logger().info('LTL Planner: Suffix plan: ' + str(self.suf_plan))