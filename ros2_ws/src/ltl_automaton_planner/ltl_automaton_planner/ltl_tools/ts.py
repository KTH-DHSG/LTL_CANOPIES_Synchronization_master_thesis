...
class TSModel(DiGraph):
    ...
    def __init__(self, ros_node, state_models):
        """
    ...
    def build_full(self):
        ...
        self.ros_node.get_logger().info("LTL Planner: full model constructed with %d states and %s transitions" %(len(self.nodes()), len(self.edges())))
        self.ros_node.get_logger().info("LTL Planner: initial state in TS is %s" %str(self.graph['initial']))
