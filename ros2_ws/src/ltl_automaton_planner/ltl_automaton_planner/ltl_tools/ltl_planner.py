...
class LTLPlanner(object):
    ...
    def optimal(self, style='static'):
        self.ros_node.get_logger().info("LTL Planner: --- Planning in progress ("+style+") ---")
        self.ros_node.get_logger().info("LTL Planner: Hard task is: "+str(self.hard_spec))
        self.ros_node.get_logger().info("LTL Planner: Soft task is: "+str(self.soft_spec))

        if style == 'static':
            # full graph construction
            self.product = ProdAut(self.ros_node, self.ts, mission_to_buchi(self.ros_node, self.hard_spec, self.soft_spec), self.beta)
            ...
            if self.product:
                ...
                self.ros_node.get_logger().error("LTL Planner: \"ready\" planning was requested but product graph was never built, aborting...")
                return False
            ...
            if self.product:
                ...
                self.ros_node.get_logger().error("LTL Planner: \"on-the-fly-initial\" planning was requested but product graph was never built, aborting...")
                return False
            ...
            if self.product:
                ...
                self.ros_node.get_logger().error("LTL Planner: \"on-the-fly-task\" planning was requested but product graph was never built, aborting...")
                return False

        if self.run == None:
            self.ros_node.get_logger().error("LTL Planner: No valid plan has been found! Check you FTS or task")
            return False

        self.ros_node.get_logger().info("LTL Planner: --- Planning successful! ---")
        self.ros_node.get_logger().debug("Prefix states: "+str([n for n in self.run.line]))
        self.ros_node.get_logger().debug("Suffix states: "+str([n for n in self.run.loop]))
    ...
    def replan(self):
        ...
        new_run = improve_plan_given_history(self.ros_node, self.product, self.trace)
