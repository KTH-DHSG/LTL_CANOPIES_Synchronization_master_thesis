...
#===========================================
def dijkstra_plan_networkX(ros_node, product, gamma=10):
    ...
    if runs:
        ...
        run = ProdAut_Run(ros_node, product, prefix, precost, suffix, sufcost, precost+gamma*sufcost)
        ros_node.get_logger().debug('Dijkstra_plan_networkX done within %.2fs: prefix cost %.2f, sufix cost %.2f' %(time.time()-start, precost, sufcost))
        return run, time.time()-start
     
    ros_node.get_logger().error('No accepting run found in optimal planning!')
    return None, None


def dijkstra_plan_optimal(ros_node, product, gamma=10, start_set=None):
    start = time.time()
    ...
    if runs:
        ...
        ros_node.get_logger().debug('optimal_dijkstra_olf done within %.2fs: precost %.2f, sufcost %.2f' %(time.time()-start, precost, sufcost))
        return run, time.time()-start

    ros_node.get_logger().error('No accepting run found in optimal planning!')


def dijkstra_plan_bounded(ros_node, product, time_limit=3, gamma=10):
    start = time.time()
    ros_node.get_logger().debug('dijkstra plan started!')
    runs = {}
    accept_set = product.graph['accept']
    init_set = product.graph['initial']
    ros_node.get_logger().debug('number of accepting states %d' %(len(accept_set)))
    ros_node.get_logger().debug('number of initial states %d' %(len(init_set)))
    loop_dict = {}
    for init_prod_node in init_set:
        for (prefix, precost) in dijkstra_targets(product, init_prod_node, accept_set):
            #print('accept node reached %s' %(str(prefix[-1])))
            ...
            if time.time()-start > time_limit:  # time limit has reached
                if runs:
                    prefix, precost, suffix, sufcost = min(runs.values(), key = lambda p: p[1] + gamma*p[3])
                    run = ProdAut_Run(ros_node, product, prefix, precost, suffix, sufcost, precost+gamma*sufcost)
                    ros_node.get_logger().debug('optimal_dijkstra done within %.2fs: precost %.2f, sufcost %.2f' %(time.time()-start, precost, sufcost))
                    return run, time.time()-start
...
def improve_plan_given_history(ros_node, product, trace):
    new_initial_set = prod_states_given_history(product, trace)
    if new_initial_set:
        new_run, time=dijkstra_plan_optimal(ros_node, product, 10, new_initial_set)
        return new_run
    else:
        return None
...
#===========================================
def validate_and_revise_after_ts_change(ros_node, run, product, sense_info, com_info):
    new_prefix = None
    new_suffix = None
    start = time.time()
    ...
    if changed_regs:
        for (index, prod_edge) in enumerate(run.pre_prod_edges):
            ...
            if t_ts_node not in succ_prod:
                    ros_node.get_logger().error('The current plan prefix contains invalid edges, need revision!')
        ...
        for (index, prod_edge) in enumerate(run.suf_prod_edges):
            ...
            for prod_node_to, weight in product.graph['ts'].fly_successors(f_ts_node):
                succ_prod.add(prod_node_to)
            if t_ts_node not in succ_prod:
                    ros_node.get_logger().error('The current plan suffix contains invalid edges, need revision!')
        ...
        if new_prefix or new_suffix:
            ...
            run.output(product)
            ros_node.get_logger().info('validate_and_revise_after_ts_change done in %.2fs' %(time.time()-start))
        else:
            ros_node.get_logger().error('Local revision failed!')
            return False
