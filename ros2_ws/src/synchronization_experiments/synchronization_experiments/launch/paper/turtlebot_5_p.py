...
def generate_launch_description():
    ...
    dynamic_obstacles = ['/rosie0', '/rosie1', '/rosie2', '/turtlebot1', '/turtlebot2', '/turtlebot3', '/turtlebot4', '/turtlebot6', '/basket']#, '/turtlebot7']
    action_node= Node(
            package='ltl_automaton_synchronization',
            executable='auto_actions',
            name='auto_actions',
            namespace='turtlebot5',
            emulate_tty=True,
            output='screen',
            parameters=[
                {'motion_action_dictionary_path': os.path.join(get_package_share_directory('synchronization_experiments'), 'config/paper', 'turtlebot5.yaml')},
                {'agents': agents},
    ...
    planner_node= Node(
            ...
            output='screen',
            parameters=[
                #{'hard_task':'[]<> (patrol && p14 && <> (patrol && p11 && <> (check_connection && c1)))' },
                {'hard_task':'X wait && []<> (patrol && p3 && <> (patrol && p8 && <> group))' },
                {'soft_task': ""},
